...
% in this case we generate one bernoulli graphs, and another graph which is
% just like the first one with the nodes permuted.
% it is easy to see with a look that the two graphs are the same when n is small
...
%% however, when n is large, this is not so easy
...
%% if the graphs are small, it is possible to just find if this is the case or not. This is called the isomorphism problem
...
%% note that if the graphs are not isomorphic, than we will not be able to find a permutation of the nodes of A such that it equals B
n = 6;
A = (rand(n) > 0.5); A = triu(A,1) + triu(A,1)';
...
% a more compicated problem, known to be hard, is that of determining of close two graphs are of being isomorphic
...
% is large, the graphs are more dissimilar


%$ there are two main approachces to comparingn two graph. Alignment based
%methods, and alignment-free methods. As the same implies, alignment based
%methods first try to align the nodes of the two graphs as well as
%possible, via some permutation of nodes, then, once the nodes are
%aligned, it compares the edges between each pair of nodes in the two
%graphs. Alignment free methods, do not do this.
...
%% note however, that it is possible that wo graphs are not-isomorphic, and
% however have the same specturm. So, when comparing spectra, we should keep in
% mind what co-spectrality actually implies
% isomorphism => co-spectrality, but not the other way around
...
% Note that if P is a permutation, for example,  [6     4     5     1     3
% 2], and A is an adjacancy matrix, then doing A(P,P), is the same as doing
...
% such that work with a more tractable set of matrices.
...
% (corresponding to a maximum maching problem), which always outputs a
% permutaion matrix
...
% in this case it is a bit harder to get a permutation from the orthogonal
% matrix
...
% other words, we want it to satisfy D >= 0, D = 0 if and only if graph_1 ~
% graph_2, D(graph_1,graph_2) = D(graph_2,graph_1), and D(graph_1,graph_2)
% + D(graph_2,graph_3) >= D(graph_1,graph_3)
%
% the reason for wanting this is kind of obvious, if graph 1 and graph 2
% are simila, and graph 2 and graph 3 are also similar, then graph 1 and
% graph 3 should also be similar
%
%
% futhermore, the metric property allows to speedup several machine
% learning algorithms, and allows to improve the performance in other
% learning algorithms. See J. Bento, S. Ioannidis, A Family of tractable graph distances, SDM 2018
% and also see S. Safavi, J. Bento, n-metrics for multiple graph alignment, 2018
%
...
% in fact, for many choices of domain for M, this is the case. See papers
% above
%
...
% node x in G_1 to y in G_2. And D(G_2,G_3) might match nodes y in G_2 to z
...
%% A different approach is to also relax both the permutations, and the alignment consistency constraint
